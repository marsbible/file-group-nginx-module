...
#define arg_len(a, decode) {a, sizeof(a)-1, decode} 
struct app_arg_item {
    const char *str;
    size_t len;
...
typedef struct app_arg_item app_arg_item;
...
void *ngx_trie_match(ngx_trie_t *trie, ngx_str_t *str, ngx_int_t *version_pos,
    ngx_uint_t mode);
ngx_uint_t ngx_extract_args(ngx_trie_t *trie, const app_arg_item *app_args, ngx_str_t *str, ngx_str_t *args);
